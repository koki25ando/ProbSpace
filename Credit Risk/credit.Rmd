...
for (i in 2:10){
    ...
    accuracy_list[[i]] = confusionMatrix(as.factor(train$Y), as.factor(train$pred))$overall[1]
}
toc()
```

```{r}
accuracy_table = data.frame(trial = do.call(rbind, nth_list), Accuracy = do.call(rbind, accuracy_list))
accuracy_table %>% 
    ggplot(aes(trial, Accuracy, label = Accuracy)) +
    geom_bar(stat = "identity", aes(fill = trial)) +
    geom_label()
accuracy_table %>% 
    ggplot(aes(reorder(trial, -Accuracy), Accuracy, label = Accuracy)) +
    geom_bar(stat = "identity", aes(fill = trial)) +
    geom_label()

accuracy_table %>% 
    ggplot(aes(reorder(trial, -Accuracy), Accuracy, color = trial)) +
    geom_point()
```
